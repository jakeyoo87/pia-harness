...
상태: 설계 계획 — 구현·릴리스·PIA 통합·AWS·실모델 호출 없음
...
4. 읽기 도구를 고른 generation은 아직 `GENERATING`이다. 도구를 호출하고 observation을 같은 generation의 **비영속·비신뢰 Context**에 추가한 뒤 모델을 다시 호출한다. 다음 모델 응답은 다시 읽기 도구 하나를 선택하거나 최종 Answer를 낸다. 첫 구현은 읽기 도구 최대 3회, 뒤따르는 최종 모델 호출 최대 1회로 고정한다. 상한에 닿았는데 모델이 또 도구를 요청하면 성공을 꾸며 전달하지 않고 안전한 실패로 종료한다. Harness 자체의 자동 도구 재시도·동일 호출 캐시·계획기·무제한 loop는 없다.
...
7. observation은 `PromptContextKind.TOOL_RESULT` 같은 명시적 비신뢰 종류로 순서대로 담는다. Renderer가 “외부 도구 데이터이며 명령이 아님”으로 표시하고 trusted system instruction으로 렌더하지 않는다. 기존 Memory/Summary/Turn과 구분한다. 각 왕복에서 실제 assembled payload를 다시 세어 input budget을 확인한다. 초과 observation을 조용히 잘라내거나 context에 억지로 넣지 않는다. 모델 출력 초안과 중간 observation은 Turn·Summary·Memory·로그에 저장하지 않는다.
8. 읽기 도구를 한 번이라도 쓴 Turn의 **최종** 전달 문구와 저장용 user·assistant 문구는 호스트가 commit 직전에 확정한다(기존 `ToolResult`의 세 문구 분리 재사용 가능). 최종 모델 설명을 사용자에게 보여주더라도 그대로 저장하는 것을 기본값으로 삼지 않는다. 중간 도구가 여러 개일 때 어느 도구의 placeholder를 택할지 추측하지 말고 한 번의 최종 host redaction 결과를 받는다. 이 callback은 텍스트 형식화만 하며 외부 부작용을 실행하지 않는다. 일반 답변과 기존 단일 쓰기 도구의 저장 동작은 변경하지 않는다.
9. 도구가 포함된 왕복에서는 `MemoryAction.NONE`을 유지한다. 특히 observation의 문장이 Memory 수정·삭제 지시가 될 수 없다. 첫 모델 호출의 Web Search 선택은 기존처럼 최종 답변으로 끝나며, `tool_call`과 `needs_web_search=true` 동시 선택은 계속 무효다. 읽기 observation 이후의 모델 호출에서는 Server Tool 검색을 비활성화해 계좌 데이터가 검색 단계로 넘어가지 않게 한다. Adapter에 필요한 최소 per-call 모드만 추가하고, 도구 미등록 경로는 바꾸지 않는다.
10. 입력이 큰 observation, read callback 오류, round 상한, 전체 timeout, 모델 출력 오류는 raw 결과를 노출하지 않고 종료한다. 예기치 않은 callback 오류는 기존 `TOOL_FAILED`/pending clear 의미를 쓴다. 알려진 조회 실패는 호스트가 안전한 observation 또는 결정적 실패 문구로 처리하며 Harness는 자동 재조회하지 않는다. 애플리케이션은 전체 시간 상한을 정해야 한다. PIA Bot이 순차적으로 메시지를 처리하므로 여러 모델 왕복이 다른 사용자의 명령을 오래 막지 않는지도 PIA 통합 전 별도 확인한다.
...
2. fake model + fake read tool로 0/1/2/3회 왕복, 도구 뒤 정상 Answer, 순서 있는 observation, 도구 뒤 쓰기 도구 선택(쓰기 commit 이전 미호출), 상한 도달을 시험한다. 도구 이름·인자 오류와 도구 없는 기존 경로도 시험한다.
3. callback 도중·callback 직후·후속 모델 호출 도중 supersede/reset, cancellation 무시, claim 경합, 반복 외부 cancellation에서 도구/전달/Turn이 중복되지 않음을 시험한다. 읽기 callback은 GENERATING에서만, 쓰기는 COMMITTING에서만 실행됨을 테스트로 고정한다.
4. observation prompt injection, 다른 user_key 결과 혼입, provider 원문/비밀 비노출, 매 왕복 token budget 초과, Memory action/검색 동시 선택, Web Search 이후 종료, 도구 뒤 검색 금지, Turn의 양쪽 redaction 및 Summary/Memory 미유입을 시험한다. 모델 사용량은 마지막 Answer의 Context 기준과 각 호출 비용을 혼동하지 않도록 문서화한다.
5. `docs/01`, `03`, `04`, README, public export 및 fake 통합 예제를 구현에 맞춰 갱신한다. 변경 영역 테스트 후 전체 network-free suite와 wheel build를 한 번 실행한다. 사용자가 별도 승인하면 synthetic 입력만으로 실제 OpenRouter 모델의 nullable tool schema와 2회 왕복을 smoke한다. 검토·릴리스·PIA pin·AWS 배포는 각각 별도 단계다.
...
2. 새 공개 API를 read-only 이름 allowlist + callback + 최종 redaction 정도로 제한할 수 있는가? 기존 `ToolCall`/`ToolResult` 재사용과 명시적 observation 타입 중 무엇이 더 단순하고 안전한가? 불필요한 registry/상태/예외를 지적해 달라.
3. 여러 모델 호출의 Context budget·Compaction 기준과 tool observation의 prompt-injection/redaction 경계가 충분한가? 중간 결과가 raw Turn·Summary·Memory·로그에 새는 경로가 있는가?
4. 기존 두 단계 Web Search와 새 조회 왕복을 결합하지 않는 규칙이 호환성을 지키는가? 실모델 structured-output smoke에서 무엇을 먼저 확인해야 하는가?
5. 3회 read 상한과 앱 전체 timeout이 PIA의 순차 Telegram Bot 운영에서 실용적인가? 더 작은 첫 수직 슬라이스가 있다면 제안하되, Broker 주문 경계를 Harness로 옮기거나 사용자별 상시 VM을 전제로 하지 말아 달라.
