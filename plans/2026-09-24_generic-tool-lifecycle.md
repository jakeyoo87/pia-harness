...
- 호스트가 generation마다 주는 선택적 짧은 `ephemeral_tool_context(user_key)`를 Context 예산에 포함한다. 이 값은 **비영속·비신뢰 데이터**로 별도 표시하며 trusted SYSTEM이나 tool 지침으로 승격하지 않는다. 모델에게 현재 진행 중인 앱 도구 초안을 알려주지만 Turn·Summary·Memory에는 저장하지 않는다. 값 부재 시 기존 Context 구조가 정확히 유지된다.
...
- 도구 callback의 알려진 조회 실패·주문 미확정은 호스트가 안전한 결과 문구로 반환한다. 예기치 않은 callback 예외는 일반 답변으로 대체하지 않고 `TOOL_FAILED`(이름은 구현 시 확정)로 반환한다. 이때 `clear_pending=True`, 전달·Turn 저장 없음으로 다음 입력의 조용한 자동 재실행을 막는다. 호출 앱은 결정적 실패 안내와 별도 상태조회 경로를 제공하며 Harness는 원문/인자를 로그에 남기지 않는다. `ConversationAbandoned`는 기존 ABANDONED 동작을 유지한다.
...
2. fake 모델·fake tool로 정상 read/write, schema 거부, tool+Web Search 동시 요청, supersede, commit 중 새 입력, callback 오류 시 `TOOL_FAILED`/pending clear, 프로세스 재생성에 준하는 동일 source ID 재호출, 전달 실패, Turn 저장 실패, 두 방향 redaction, ephemeral context의 비신뢰·비영속·예산 계산, Memory/Compaction 비유입을 시험한다. 실제 증권사·AWS·사용자 데이터는 쓰지 않는다.
